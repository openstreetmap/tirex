...
Defaults to F</usr/lib/tirex/tiles>.
...
=head2 RENDERD OPTIONS

=over 3

=item B<mapnik_fontdir>
...
throw an exception. Defaults to F</usr/lib/mapnik/fonts>.

=item B<mapnik_fontdir_recurse>

Set this to I<1> if you want fonts loaded recursively from directories
inside the I<mapnik_fontdir> directory. Defaults to I<0>.

=item B<mapnik_mapdir>

The directory where the map files for rendering reside. The map files must have 
a file extension of .xml, and all .xml files present in this directory will be 
loaded as map styles. Tirex will only process requests that have a "map" parameter
matching one of these files. Defaults to I</etc/mapnik-osm-data>.

=item B<mapnik_plugindir>

The directory where the Mapnik rendering engine loads its plugins from. 
Defaults to F</usr/lib/mapnik/input>.

=item B<renderd_processes>

The number of renderd processes to start. This should be the same, or slightly 
smaller, than the maximum I<maxprocs> value used in a I<bucket> section. If you have
more processes than I<maxprocs> then they will never be assigned a job; if you 
have less processes than I<maxprocs> then the master will hand out more requests
than can be processed. Defaults to I<5>.
...
A standard Tirex configuration file:
