...
graph_scale no
graph_period $per
graph_info Number of metatile requests rendered.
EOF
    ...
    foreach my $zoomrange (@{$self->{'zoomranges'}})
    {
        ...
        $config .= sprintf("rc_%s.info Zoomlevel %s\n", $id, $zoomrange->to_s());
        $config .= sprintf("rc_%s.label %s\n",          $id, $zoomrange->get_name());
        $config .= sprintf("rc_%s.type DERIVE\n",       $id);
        $config .= sprintf("rc_%s.draw %s\n",           $id, $type);
    }
